...
function HeroBlock({ data }: { data: ResultData }) {
  ...
  const pct = hasKeywords ? found / total : 0;
  const dashOffset = CIRCLE_C * (1 - pct);
  ...
  return (
    <div className="relative bg-[#111] border border-[#222] rounded-2xl overflow-hidden">
      {/* Círculos decorativos integrados en la composición */}
      <div
        aria-hidden
      ...
      <div
        aria-hidden
        className="absolute -bottom-20 left-32 w-72 h-72 rounded-full"
          ...
          {hasKeywords ? (
            ...
            <>
              <div>
                ...
                <p className="mt-1.5 text-sm text-white/50">
                  {cargo && empresa
                    ? `Adaptado para ${cargo} en ${empresa}`
                    : cargo
                    ? `Adaptado para ${cargo}`
...
// ── CV text parsing (mirrors CVDocument.tsx) ──────────────────────────
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    switch (role) {
      case "name":
        elements.push(
          <p key={i} style={{ fontFamily: "Helvetica Neue, Helvetica, Arial, sans-serif" }}
            className="text-center font-bold uppercase tracking-widest text-[14px] text-white mb-1.5 leading-tight">
            {t}
          </p>
        );
        break;
      case "title":
        elements.push(
          <p key={i} style={{ fontFamily: "Helvetica Neue, Helvetica, Arial, sans-serif" }}
            className="text-center text-[11px] text-white/55 mb-0.5 leading-snug">
            {t}
          </p>
        );
        break;
      case "contact":
        elements.push(
          <p key={i} style={{ fontFamily: "Helvetica Neue, Helvetica, Arial, sans-serif" }}
            className="text-center text-[10px] text-white/45 mb-2.5 leading-snug">
            {t}
          </p>
        );
        if (!ruleDone) {
      ...
      case "section":
        ...
        elements.push(
          <p key={i} style={{ fontFamily: "Helvetica Neue, Helvetica, Arial, sans-serif" }}
            className="text-[9.5px] font-bold uppercase tracking-wider text-white/65 border-b border-white/12 pb-1 mt-4 mb-2 leading-none">
            {t}
          </p>
        );
        break;
      ...
      case "body":
        elements.push(
          <p key={i} style={{ fontFamily: "Helvetica Neue, Helvetica, Arial, sans-serif" }}
            className="text-[10px] text-white/80 leading-relaxed mb-0.5">
            {t}
          </p>
        );
        break;
...
export default function ResultadoPage() {
  ...
  async function copyActive() {
    ...
    await navigator.clipboard.writeText(
      activeTab === "cv" ? data.cv_adaptado : data.carta_presentacion
    );
    setCopied(true);
        ...
        <main className="flex-1 px-6 sm:px-10 py-10 sm:py-14 relative">
          <div className="w-full max-w-[960px] mx-auto flex flex-col gap-8">
            ...
            <div className="grid grid-cols-1 md:grid-cols-[5fr_3fr] gap-6 items-start">
              ...
              {/* COLUMNA IZQUIERDA — Card con tabs CV / Carta */}
              <div className="bg-[#111] border border-[#222] rounded-2xl flex flex-col overflow-hidden">
                ...
                <div className="relative flex-1 min-h-0">
                  ...
                  <div ref={scrollRef} className="overflow-y-auto max-h-[580px] md:max-h-[660px] px-7 py-5">
                    {activeTab === "cv"
                      ? <CVPreviewElements text={data.cv_adaptado} />
                      : <p className="text-sm text-white/90 leading-relaxed whitespace-pre-wrap">{data.carta_presentacion}</p>
                    }
                  </div>
              ...
              <div className="flex flex-col gap-2 pt-1">
                ...
                {data.principales_cambios.length > 3 && (
                  <button
                    type="button"
                  ...
                  >
                    {cambiosExpanded
                      ? "Ver menos ↑"
                      : `Ver ${data.principales_cambios.length - 3} más ↓`}
                  </button>
                )}
              </div>
            </div>
