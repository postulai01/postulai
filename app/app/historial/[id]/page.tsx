...
function HeroBlock({ post }: { post: Postulacion }) {
  ...
  const kvTotal = post.keywords_totales ?? 0;
...
export default function HistorialIdPage() {
  ...
  async function handleDownloadCartaPDF() {
    ...
    try {
      ...
      const lines = doc.splitTextToSize(post.carta_presentacion, pageWidth);
  ...
  async function handleDownloadCartaWord() {
    ...
    try {
      ...
      const children = post.carta_presentacion.split("\n").map(line =>
        new Paragraph({
  ...
  const cartaWordCount = post.carta_presentacion.trim().split(/\s+/).filter(Boolean).length;
  const cvText = post.cv_adaptado;
